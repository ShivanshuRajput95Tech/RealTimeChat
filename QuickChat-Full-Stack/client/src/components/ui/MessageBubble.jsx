...
const MessageBubble = ({ msg, isMine, peerAvatar, myAvatar }) => {
  ...
  const bubbleClass = isMine
    ? 'bg-gradient-to-br from-violet-600 via-violet-500 to-cyan-500 text-white rounded-br-md shadow-[0_18px_40px_rgba(79,70,229,0.24)]'
    : isDark
      ? 'bg-slate-800/90 text-slate-100 rounded-bl-md border border-white/6 shadow-[0_18px_34px_rgba(15,23,42,0.45)]'
      : 'bg-white text-slate-800 rounded-bl-md border border-slate-200 shadow-[0_12px_30px_rgba(15,23,42,0.08)]';

  return (
    <div className={`flex max-w-full items-end gap-2.5 ${isMine ? 'justify-end' : 'justify-start'}`}>
      {!isMine && (
        <img
      ...
      <div className={`flex max-w-[min(100%,28rem)] flex-col ${isMine ? 'items-end' : 'items-start'}`}>
        <div className={`rounded-[22px] px-4 py-3 text-sm break-words transition-all duration-200 ${bubbleClass}`}>
          {showImage && (
            <button
              type='button'
